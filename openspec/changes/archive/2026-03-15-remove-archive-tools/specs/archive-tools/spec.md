...
**Reason**: Archive 工具对本地资源消耗大，处理大文件时占用大量 CPU 和内存。根据 MCP 优先架构，此类资源密集型功能应通过 MCP Server 提供，以保持 Niuma 核心轻量级。
**Migration**: 用户可通过 MCP 对接专业的压缩工具（如 @modelcontextprotocol/server-filesystem）。配置示例：
...
**Reason**: Extract 工具对本地资源消耗大，处理大文件时占用大量 CPU 和内存。根据 MCP 优先架构，此类资源密集型功能应通过 MCP Server 提供，以保持 Niuma 核心轻量级。
**Migration**: 用户可通过 MCP 对接专业的压缩工具（如 @modelcontextprotocol/server-filesystem）。配置示例：
