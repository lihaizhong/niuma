...
SubagentExecutor 必须封装子智能体的 LLM 循环和工具调用逻辑，与 SubagentManager 解耦。
...
Executor 必须通过 AbortSignal 支持任务取消。
...
Executor 必须构建子智能体的系统提示词，包含工作空间和可用工具信息。
