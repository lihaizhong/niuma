...
- **配置文件（1 个）**：
  - `.vscode/settings.json` - 添加新的拼写检查词汇
...
1. 安装新的 npm 依赖：
   ```bash
   pnpm add archiver adm-zip tar ping js-yaml fs-extra deepmerge
...
6. 进行代码质量改进：
   - 替换所有 `fs` 和 `fs/promises` 为 `fs-extra`（12 个文件）
   - 规范化所有 import 语句（25 个文件）
   - 消除所有 `any` 类型（26 处）
   - 使用 deepmerge 库替代自定义深度合并实现

7. 更新 VS Code 设置：
   - 在 `.vscode/settings.json` 中添加新的拼写检查词汇：`backlink`、`backlinks`、`frontmatter`、`sqliteai`

8. 添加核心模块测试：
   - context-builder.test.ts
   - tool-registry.test.ts

9. 运行类型检查和测试，确保所有修改正确
