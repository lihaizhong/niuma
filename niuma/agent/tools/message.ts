...
class MessageQueue {
  ...
  enqueue(message: MessageHistory): void {
    ...
    const priority =
      priorityMap[message.priority as keyof typeof priorityMap] || 2;
  ...
  private async sendMessage(message: MessageHistory): Promise<void> {
    try {
      ...
      logger.info(
        { messageId: message.messageId, channel: message.channel },
        "消息已发送",
      );
    } catch (error) {
      ...
      logger.error(
        { messageId: message.messageId, error: message.error },
        "消息发送失败",
      );
    }
  }
