...
export class FileSearchTool extends BaseTool {
  ...
  readonly description = '在文件中使用正则表达式搜索内容。支持大小写敏感/不敏感选项和匹配数量限制。'
  readonly parameters = z.object({
    path: z.string().describe('文件路径'),
    pattern: z.string().describe('搜索模式（正则表达式）'),
    caseSensitive: z.boolean().optional().default(false).describe('大小写敏感（默认 false）'),
    maxMatches: z.number().int().positive().optional().default(100).describe('最大匹配数（默认 100）'),
  })
  ...
  async execute(args: {
    ...
    const { path, pattern, caseSensitive = false, maxMatches = MAX_MATCHES } = args
    const resolvedPath = resolvePath(path)
    ...
    try {
      ...
      if (!stats.isFile()) {
        throw new ToolExecutionError(this.name, `路径不是文件: ${resolvedPath}`)
      }
      ...
      // 搜索匹配的行
      const matches: Array<{ lineNumber: number; line: string; matches: string[] }> = []
      let totalMatches = 0
      ...
      throw new ToolExecutionError(this.name, `搜索文件失败: ${(error as Error).message}`)
    }
  }
}
  ...
  async execute(args: { source: string; dest: string }): Promise<string> {
    ...
    try {
      ...
      if (!sourceStats.isFile()) {
        throw new ToolExecutionError(this.name, `源路径不是文件: ${resolvedSource}`)
      }
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        throw new ToolExecutionError(this.name, `源文件不存在: ${resolvedSource}`)
      }
      throw new ToolExecutionError(this.name, `移动文件失败: ${(error as Error).message}`)
    }
  }
}
...
export class FileCopyTool extends BaseTool {
  ...
  async execute(args: { source: string; dest: string }): Promise<string> {
    ...
    try {
      ...
      throw new ToolExecutionError(this.name, `复制文件失败: ${(error as Error).message}`)
    }
  }
}
...
export class FileDeleteTool extends BaseTool {
  ...
  async execute(args: { path: string; confirm: boolean }): Promise<string> {
    ...
    // 安全检查
    if (!confirm) {
      throw new ToolExecutionError(this.name, '删除操作需要确认，请设置 confirm 参数为 true')
    }
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'EACCES') {
        throw new ToolExecutionError(this.name, `无权限删除文件: ${resolvedPath}`)
      }
      throw new ToolExecutionError(this.name, `删除文件失败: ${(error as Error).message}`)
    }
  }
}
...
export class FileInfoTool extends BaseTool {
  ...
  async execute(args: { path: string }): Promise<string> {
    ...
    try {
      ...
      throw new ToolExecutionError(this.name, `获取文件信息失败: ${(error as Error).message}`)
    }
  }
}
...
export class DirCreateTool extends BaseTool {
  ...
  readonly parameters = z.object({
    path: z.string().describe('目录路径'),
    recursive: z.boolean().optional().default(true).describe('递归创建（默认 true）'),
  })
  ...
  async execute(args: { path: string; recursive?: boolean }): Promise<string> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'EACCES') {
        throw new ToolExecutionError(this.name, `无权限创建目录: ${resolvedPath}`)
      }
      throw new ToolExecutionError(this.name, `创建目录失败: ${(error as Error).message}`)
    }
  }
}
...
export class DirDeleteTool extends BaseTool {
  ...
  readonly parameters = z.object({
    path: z.string().describe('目录路径'),
    recursive: z.boolean().optional().default(false).describe('递归删除（默认 false）'),
    confirm: z.boolean().describe('必须设置为 true 才能执行删除操作'),
  })

  async execute(args: { path: string; recursive?: boolean; confirm: boolean }): Promise<string> {
    ...
    for (const protectedPath of protectedPaths) {
      if (resolvedPath === protectedPath || resolvedPath.startsWith(protectedPath + '/')) {
        throw new ToolExecutionError(this.name, `不允许删除受保护路径: ${resolvedPath}`)
      }
    }
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'EACCES') {
        throw new ToolExecutionError(this.name, `无权限删除目录: ${resolvedPath}`)
      }
      throw new ToolExecutionError(this.name, `删除目录失败: ${(error as Error).message}`)
    }
  }
}
