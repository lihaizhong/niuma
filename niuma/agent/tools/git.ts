 ...
 * Git 命令执行辅助函数
 */
async function execGit(args: string[], cwd: string = process.cwd()): Promise<string> {
...
export const gitStatusTool = new SimpleTool({
  name: 'git_status',
  description: '查看 Git 仓库状态，包括修改的文件、暂存的文件、未跟踪的文件和分支信息。',
  parameters: z.object({}),
  handler: async () => {
    ...
    if (!(await isGitRepository(cwd))) {
      throw new ToolExecutionError('git_status', '当前目录不是 Git 仓库，请先初始化 Git 仓库（git init）')
    }
    ...
    try {
      ...
      throw new ToolExecutionError('git_status', `获取 Git 状态失败: ${(error as Error).message}`)
    }
  },
})
...
export const gitCommitTool = new SimpleTool({
  name: 'git_commit',
  description: '创建 Git 提交。可以提交所有暂存的更改，或指定文件路径提交特定文件。',
  parameters: z.object({
    message: z.string().min(1).describe('提交消息（必填）'),
    files: z.array(z.string()).optional().describe('要提交的文件路径列表（可选，不指定则提交所有暂存的更改）'),
  }),
  handler: async (args) => {
    ...
    try {
      ...
      if (!status) {
        throw new ToolExecutionError('git_commit', '没有暂存的更改可以提交。请先使用 git add 暂存文件，或检查文件状态。')
      }
      ...
      throw new ToolExecutionError('git_commit', `提交失败: ${(error as Error).message}`)
    }
  },
})
  ...
  handler: async (args) => {
    ...
    try {
      ...
      if (errorMessage.includes('authentication')) {
        throw new ToolExecutionError('git_push', '认证失败，请检查您的凭据或使用 SSH')
      }
      if (errorMessage.includes('behind')) {
        throw new ToolExecutionError('git_push', '本地分支落后于远程，请先拉取最新更改（git pull）')
      }
...
export const gitPullTool = new SimpleTool({
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // 处理常见错误
      if (errorMessage.includes('authentication')) {
        throw new ToolExecutionError('git_pull', '认证失败，请检查您的凭据或使用 SSH')
      }
      if (errorMessage.includes('conflict')) {
        throw new ToolExecutionError('git_pull', '合并冲突，请手动解决冲突后继续')
      }
      if (errorMessage.includes('does not exist')) {
...
export const gitBranchTool = new SimpleTool({
  ...
  parameters: z.object({
    operation: z.enum(['list', 'create', 'delete']).describe('操作类型：list（列出）、create（创建）、delete（删除）'),
    ...
    try {
      switch (operation) {
        case 'list': {
          ...
          let result = '所有分支:\n'
          result += branches.map((b) => (b === current.trim() ? `* ${b} (当前)` : `  ${b}`)).join('\n')
        ...
        case 'create': {
          if (!name) {
            throw new ToolExecutionError('git_branch', '创建分支需要指定分支名称')
          }
        ...
        case 'delete': {
          if (!name) {
            throw new ToolExecutionError('git_branch', '删除分支需要指定分支名称')
          }

          // 检查是否为当前分支
          const current = await execGit(['branch', '--show-current'], cwd)
          if (current.trim() === name) {
            throw new ToolExecutionError('git_branch', '无法删除当前分支，请先切换到其他分支')
          }
        ...
        default:
          throw new ToolExecutionError('git_branch', `不支持的操作: ${operation}`)
      }
    } catch (error) {
      if (error instanceof ToolExecutionError) {
        throw error
      }
      throw new ToolExecutionError('git_branch', `分支操作失败: ${(error as Error).message}`)
    }
  },
})
...
export const gitLogTool = new SimpleTool({
  ...
  parameters: z.object({
    count: z.number().int().positive().max(MAX_LOG_COUNT).optional().describe('显示的提交数量（默认：10，最大：50）'),
    branch: z.string().optional().describe('分支名称（默认：当前分支）'),
  }),
  handler: async (args) => {
    ...
    try {
      ...
      if (targetBranch) {
        ...
        if (!existing.trim()) {
          throw new ToolExecutionError('git_log', `分支 "${targetBranch}" 不存在`)
        }
      }

      // 获取提交历史
      const format = '%h|%an|%ad|%s'
      const output = await execGit(['log', `-${count}`, `--pretty=format:${format}`, targetBranch], cwd)
      ...
      // 检查是否有更多提交
      const totalCount = await execGit(['rev-list', '--count', targetBranch], cwd)
      ...
      throw new ToolExecutionError('git_log', `获取提交历史失败: ${(error as Error).message}`)
    }
  },
})