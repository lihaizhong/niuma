 ...
 * 检查命令是否危险
 */
function isDangerousCommand(command: string): { isDangerous: boolean; description?: string; requireConfirmation: boolean } {
...
export class ExecTool extends BaseTool {
  readonly name = 'exec'
  readonly description = '执行 Shell 命令。包含危险命令黑名单防护。支持工作目录、环境变量、超时控制等。'
  ...
  async execute(args: {
    ...
    const { command, args: cmdArgs = [], cwd, env, timeout = 30000, runInBackground = false, strict = false } = args

    // 检查危险命令
    const dangerCheck = isDangerousCommand(command)
    if (dangerCheck.isDangerous) {
      if (!dangerCheck.requireConfirmation) {
        throw new ToolExecutionError(this.name, `拒绝执行危险命令: ${command} (${dangerCheck.description})`)
      }
      // TODO: 实现用户确认机制（当前版本直接拒绝）
      throw new ToolExecutionError(this.name, `危险命令需要用户确认: ${command} (${dangerCheck.description})`)
    }
    ...
    try {
      ...
      const execError = error as { stdout?: string; stderr?: string; message?: string; code?: number; name?: string }
