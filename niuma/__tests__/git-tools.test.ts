...
describe('Git Tools', () => {
  ...
  describe('gitStatusTool', () => {
    it('应该在非 Git 目录中返回错误', async () => {
      ...
      try {
        await expect(gitStatusTool.execute({})).rejects.toThrow(ToolExecutionError)
  ...
  describe('gitCommitTool', () => {
    it('应该在提交消息为空时返回错误', async () => {
      ...
      try {
        await expect(gitCommitTool.execute({ message: '' })).rejects.toThrow(ToolExecutionError)
        await expect(gitCommitTool.execute({ message: '' })).rejects.toThrow('提交消息不能为空')
      } finally {
        process.chdir(originalCwd)
      }
    })
  ...
  describe('gitBranchTool', () => {
    ...
    it('应该创建新分支', async () => {
      ...
      try {
        const result = await gitBranchTool.execute({ operation: 'create', name: 'new-branch' })
    ...
    it('应该在分支已存在时返回错误', async () => {
      ...
      try {
        ...
        // 尝试创建同名分支
        await expect(
          gitBranchTool.execute({ operation: 'create', name: 'existing-branch' })
        ).rejects.toThrow(ToolExecutionError)
        await expect(
          gitBranchTool.execute({ operation: 'create', name: 'existing-branch' })
        ).rejects.toThrow('已存在')
    ...
    it('应该删除分支', async () => {
      ...
      try {
        ...
        // 删除分支
        const result = await gitBranchTool.execute({ operation: 'delete', name: 'to-delete' })
    ...
    it('应该在删除当前分支时返回错误', async () => {
      ...
      try {
        // 获取当前分支名
        const currentBranch = execSync('git branch --show-current', { cwd: testRepoPath }).toString().trim()
  ...
  describe('gitLogTool', () => {
    it('应该在非 Git 目录中返回错误', async () => {
      ...
      try {
        await expect(gitLogTool.execute({ count: 10 })).rejects.toThrow(ToolExecutionError)
