...
export class OpenAIProvider implements LLMProvider {
  ...
  async chat(options: ChatOptions): Promise<LLMResponse> {
    ...
    try {
      ...
      // 调用 API
      const response = await this.client.invoke(langchainMessages, invokeOptions)
  ...
  private _validateTools(tools: ToolDefinition[]): void {
    for (const tool of tools) {
      if (!tool.name || typeof tool.name !== 'string') {
        throw new ProviderError(this.name, '工具名称必须是非空字符串', undefined, { tool })
      }
      if (!tool.description || typeof tool.description !== 'string') {
        throw new ProviderError(this.name, `工具 ${tool.name} 必须有描述`, undefined, { tool })
      }
    }
  }
   ...
   */
  private _normalizeParameters(parameters: ToolDefinition['parameters']): Record<string, unknown> {
    // 检查是否是 Zod Schema
    if (parameters && typeof parameters === 'object' && 'safeParse' in parameters) {
  ...
  private _parseResponse(response: AIMessage): LLMResponse {
    ...
    // 解析工具调用
    const toolCalls: ToolCall[] | undefined = response.tool_calls?.map((tc) => ({
      id: tc.id || '',
      name: tc.name,
    ...
    return {
      ...
      finishReason: response.response_metadata?.finish_reason as string | undefined,
      id: response.id,
    }
  }
  ...
  private _parseStreamChunk(chunk: AIMessage): LLMStreamChunk {
    ...
    // 解析工具调用（流式中可能是部分的）
    const toolCalls: Partial<ToolCall>[] | undefined = chunk.tool_calls?.map((tc) => ({
      id: tc.id,
      name: tc.name,
      arguments: tc.args as Record<string, unknown>,
    }))
    ...
    return {
      ...
      finishReason: chunk.response_metadata?.finish_reason as string | undefined,
  ...
  private _handleError(error: unknown): ProviderError {
    ...
    if (error && typeof error === 'object') {
      ...
      // 限流错误
      if (status === 429) {
        return new ProviderError(this.name, 'OpenAI API 请求被限流，请稍后重试', 429, {
      ...
      // 无效请求
      if (status === 400) {
        return new ProviderError(this.name, `OpenAI API 请求无效: ${message}`, 400, {
          originalError: message,
      ...
      // 认证错误
      if (status === 401) {
        return new ProviderError(this.name, 'OpenAI API 认证失败，请检查 API Key', 401, {
          originalError: message,
        })
      }
      ...
      if (status && status >= 500) {
        return new ProviderError(this.name, `OpenAI API 服务器错误 (${status})`, status, {
          originalError: message,
        })
      }
    }
    ...
    if (error instanceof Error) {
      if (error.message.includes('ECONNREFUSED') || error.message.includes('ETIMEDOUT')) {
        return new ProviderError(this.name, `网络连接失败: ${error.message}`, undefined, {
